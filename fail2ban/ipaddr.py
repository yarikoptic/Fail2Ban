...
class IPAddr:
	...
	def __eq__(self, other):
		...
		return (
			...
			(self._addr == other.addr) and
			(self._family == other.family) and
	...
	def __lt__(self, other):
		return self.family < other.family or self._addr < other.addr
	...
	def ntoa(self):
		...
		return socket.inet_ntop(self._family, binary)
	...
	def getPTR(self, suffix=""):
		...
		"""
		if self.isIPv4:
			reversed_ip = self.ntoa.split(".")
			if not suffix:
				suffix = "in-_addr.arpa."
		elif self.isIPv6:
			reversed_ip = self.hexdump()
			if not suffix:
				suffix = "ip6.arpa."
		else:
			return ""

		return "%s.%s" % (".".join(reversed(reversed_ip)), suffix)
	...
	def isInNet(self, net):
		...
		return self._addr & mask == net.addr
